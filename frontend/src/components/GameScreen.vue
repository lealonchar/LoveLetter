<template>
  <div class="w-full max-w-lg space-y-4">
    ...
    <!-- Chancellor card picker (pending action) -->
    <div v-if="isMyChancellorPending" class="bg-rose-900/60 rounded-2xl p-4 space-y-3">
      <p class="text-rose-300 text-sm font-medium text-center">
        Choose a card to keep — the others return to the bottom of the deck
      </p>
      <div class="grid grid-cols-3 gap-3">
        <CardTile
            ...
            :key="card.type"
            :card="card"
            :selected="selectedChancellorCard === card.type"
    ...
    <!-- Your hand (only shown when it's your turn and no pending action) -->
    <div v-else-if="isMyTurn && myPlayer && !state.gameState.pendingAction"
         class="bg-rose-900/60 rounded-2xl p-4 space-y-3">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <CardTile
            ...
            :card="state.gameState.drawnCard"
            :selected="selectedCard === state.gameState.drawnCard.type"
    ...
    <div v-else-if="myPlayer && !myPlayer.isEliminated && !isMyChancellorPending"
...
// Guard guesses exclude Guard and Spy (value 0 and 1)
...
const optionalTargetCards = ['Prince'] // can also target self

function needsTarget(card) {
  return targetedCards.includes(card) || optionalTargetCards.includes(card)
}

// Whether it's this player's Chancellor resolution turn
